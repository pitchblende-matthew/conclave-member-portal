import { applyUnsubscribe } from "@/lib/digest";

export const dynamic = "force-dynamic";

// One-click unsubscribe from the weekly digest (token-authed, no login needed).
export async function GET(req: Request): Promise<Response> {
  const url = new URL(req.url);
  const ok = await applyUnsubscribe(Number(url.searchParams.get("u")), url.searchParams.get("t") || "");
  const heading = ok ? "Unsubscribed" : "Link invalid";
  const message = ok
    ? "You won&rsquo;t receive the weekly digest anymore. You can turn it back on anytime in your profile."
