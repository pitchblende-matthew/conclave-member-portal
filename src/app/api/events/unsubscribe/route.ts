import { applyEventUnsubscribe } from "@/lib/event-emails";

export const dynamic = "force-dynamic";

// One-click unsubscribe from event announcements + reminders (token-authed, no
// login needed).
export async function GET(req: Request): Promise<Response> {
  const url = new URL(req.url);
  const ok = await applyEventUnsubscribe(Number(url.searchParams.get("u")), url.searchParams.get("t") || "");
  const heading = ok ? "Unsubscribed" : "Link invalid";
  const message = ok
    ? "You won&rsquo;t receive event emails anymore. You can turn them back on anytime in your profile."
