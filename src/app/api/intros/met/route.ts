import { applyMetToken } from "@/lib/intros";

export const dynamic = "force-dynamic";

// One-click "we met" confirmation from an intro follow-up email (token-authed,
// no login). Marks the pair as connected so the runner stops nudging.
export async function GET(req: Request): Promise<Response> {
  const url = new URL(req.url);
  const ok = await applyMetToken(
    Number(url.searchParams.get("p")),
    Number(url.searchParams.get("u")),
    url.searchParams.get("t") || ""
  );
  const heading = ok ? "Nice work" : "Link invalid";
  const message = ok
    ? "Glad you connected — that&rsquo;s the whole point. We&rsquo;ll stop nudging you about this one."
