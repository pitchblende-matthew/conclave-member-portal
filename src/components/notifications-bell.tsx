...
export default function NotificationsBell({ items, unread }: { items: BellItem[]; unread: number }) {
  ...
  return (
    <div className="bell" onKeyDown={(e) => { if (e.key === "Escape") setOpen(false); }}>
      <button type="button" className="bell-btn" aria-label={`Notifications${unread ? `, ${unread} unread` : ""}`} aria-expanded={open} aria-haspopup="true" onClick={toggle}>
